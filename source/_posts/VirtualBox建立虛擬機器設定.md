---
title: VirtualBox建立虛擬機器設定
date: 2018-06-29 11:49:47
tags:
---

### 本文章將介紹如何在 VirtualBox 新增一台虛擬機器，使用版本為 VirtualBox 6.0.0

#### 1. 點選新增來增加虛擬機器

![ ](images/1.png)

#### 2. 名稱：命名，機器資料夾：設定好想要放虛擬機檔案的地方，類型：選擇想要的系統，版本：選擇系統版本

![ ](images/2.png)

#### 3. 設定記憶體大小

![ ](images/3.png)

#### 4. 選擇立即建立虛擬硬碟為虛擬機建立一個虛擬硬碟，如果之前有建立好的虛擬硬碟可以選擇使用現有虛擬硬碟檔案掛上去

![ ](images/4.png)

#### 5. 如果選擇立即建立虛擬硬碟則需要繼續設定硬碟，選擇 VDI 如果有需要用在其他的虛擬化軟體的環境，可以選另外二個選項

![ ](images/5.png)

#### 6. 在實體硬碟中存放介面中有說明，請依照個人需求選擇

![ ](images/6.png)

#### 7. 設定硬碟位置和大小，路徑預設是在一開始所設定的機器資料夾底下，在幫你開一個機器名稱的資料夾

![ ](images/7.png)

#### 8. 如下圖設定完後跑一下讀條後就完成了

![ ](images/8.png)
![ ](images/9.png)

#### 9. 在建好的機器列表上右鍵 → 設定可以再對機器進一步設定其他的選項

![ ](images/10.png)

#### 10. 在機器建好還沒有第一次啟動，你是可以在一般更改虛擬機器的名稱、類別和版本，當如果你已啟動虛擬機器後，就再也無法更改這些設定了

![ ](images/11.png)

#### 11. 在系統可以對記憶體跟 cpu 在次作設定

![ ](images/12.png)
![ ](images/13.png)

#### 12. 在存放裝置設定作業系統，如圖控制器：IDE 下顯示空的點選後，右邊光碟機點及光碟圖示選擇想要的作業系統

![ ](images/14.png)
![ ](images/15.png)

#### 13. 選擇作業系統的另一種方式，在建好的機器列表上右鍵 → 啟動 → 標準啟動，啟動後會問你作業系統點擊右邊資料夾圖案選擇想要的即可

![ ](images/16.png)
![ ](images/17.png)
![ ](images/15.png)
![ ](images/18.png)

#### ps. 想要刪除已建立好的虛擬機，在建好的機器列表上右鍵 → 移除，選擇移除後會詢問刪除所有檔案或是只移除，選擇指移除虛擬硬碟檔案會留著可以下次再掛載出來，下篇文章將會介紹

![ ](images/19.png)
![ ](images/20.png)

#### 到這邊建立虛擬機就完成了